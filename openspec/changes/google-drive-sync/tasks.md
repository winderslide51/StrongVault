...
- [x] 1.1 Aucune nouvelle dépendance : SDK `swift-google-drive-client` (darrarski) déjà présent
...
- [x] 2.1 Ajouter un `revisionToken: String?` opaque à `StorageMetadata` (optionnel,
      rétro-compatible ; `nil` en local — cohérent avec `kdbx-write`).
- [x] 2.2 `DriveClient.metadata(fileId:)` renseigne `revisionToken` (headRevisionId/modifiedTime).
- [x] 2.3 `GoogleDriveProvider` capture le `revisionToken` au `load()`.
- [x] 2.4 `GoogleDriveProvider.save(_:expectedRemote:)` : relire la révision distante, comparer
      ...
      « lecture seule ».
- [x] 2.5 Étendre `FakeDriveClient` : révision programmable + upload simulé qui incrémente la
      révision, pour exercer succès et conflit.

## 3. App — upload live, Keychain, UI conflit (device, build/test en CI)

- [x] 3.1 `GoogleDriveClientLive` : implémenter l'upload réel (SDK darrarski) et exposer le
      jeton de révision (headRevisionId) dans `metadata`.
- [x] 3.2 **Persistance des jetons OAuth en Keychain** avec
...
- [x] 3.3 UI de conflit : message « la base a changé sur Drive », option recharger ; l'upload
      n'écrase jamais sans révision concordante.
- [x] 3.4 **Montée de scope OAuth** : `GoogleDriveConfig.scope` passe de `drive.readonly` à
...
- [x] 4.1 Mapping métadonnées avec `revisionToken` non nul.
- [x] 4.2 Capture du jeton au `load()` réutilisable comme `expectedRemote`.
- [x] 4.3 Upload succès ⇒ nouveau `revisionToken` distinct.
- [x] 4.4 Conflit : révision distante changée ⇒ `StorageError.conflict(remote:)`, aucun upload.
- [x] 4.5 Révocation ⇒ `StorageError.accessDenied` (aucun jeton loggé).

## 5. Interop / CI

- [x] 5.1 CI Core vert via `FakeDriveClient` (aucun réseau, aucun secret). L'OAuth réel n'est
      **pas** exercé en CI.
